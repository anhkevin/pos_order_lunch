    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                ...
                @if ($orders->count() > 0 && auth()->user()->is_admin == 1)
                <br>
                <form class="form-horizontal" id="update-order-status-today" method="post" action="{{ route('admin.orders.update_status_today') }}">
                    {{ csrf_field() }}
                    {!! method_field('patch') !!}
                    <select name="status_id" id="status_id" class="dropdown-style input-field input-normal" style="width: 200px;border: 1px solid;">
                        @foreach ($statuses as $status)
                            <option value="{{ $status->id}}">{{ $status->name }}</option>
                        @endforeach
                    </select>
                    <button type="submit" id="update-order-today" class="btn btn-success">Đặt Cơm (Booked)</button>
                </form>
                <br>
                @endif
                ...
                @else
                    ...
                    <p style="padding-left: 100px;"><strong style="width: 100px;display: inline-block;">Voucher:</strong> {{ number_format($shop_info->voucher, 0, ".", ",") . "đ" }}  <button type="submit" id="update-discount-today" class="btn btn-primary" style="padding: 2px 10px;margin-left:20px;">Áp dụng Voucher + phí Ship</button></p>
